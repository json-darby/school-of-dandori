...
/**
 * Renders the main application header component.
 * 
 * Provides navigation links, manages course button click behaviours based
 * on animation completion status, and renders the project logo.
 */
export default function Header() {
  ...
  const handleCoursesClick = () => {
    if (!hasClickedSVG) {
      window.dispatchEvent(new CustomEvent('point-to-center'));
  ...
  return (
    <header className="bg-dandori-dark text-white shadow-lg sticky top-0 z-50 transition-all duration-300">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        <Link to="/" onClick={handleHomeClick} className="flex items-center gap-3 group ml-8">
          ...
          <div className="flex flex-col">
            <span className="font-serif text-4xl font-bold tracking-wide text-dandori-yellow drop-shadow-sm">
              School of Dandori
            </span>
          </div>
        </Link>
